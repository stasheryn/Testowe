...
namespace Plugins.Dropbox
{
    public static class DropboxHelper
    {
        ...
        // ja dobawyw, delete potim
        public static Action<string> DownloadCompleted;
        public static async Task ReadFileFrmDropbox(string relativePathToFile)
        {
            UnityWebRequest downloadRequest = GetRequestForFileDownload(relativePathToFile);
            var operation = downloadRequest.SendWebRequest();
            while (!operation.isDone)
            ...
            // parse
            DownloadCompleted?.Invoke(downloadRequest.downloadHandler.text);
        }
